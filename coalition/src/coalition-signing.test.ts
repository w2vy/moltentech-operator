...
/** Read a signature header, failing the test if the signer omitted it. */
function hdr(headers: Record<string, string>, name: string): string {
  const v = headers[name];
  assert.ok(v, `signer omitted ${name}`);
  return v;
}
...
function rebuild(
  ...
  return {
    method,
    path,
    slug: hdr(headers, HEADER_COALITION_SLUG),
    issuedAt: hdr(headers, HEADER_COALITION_TIMESTAMP),
    nonce: hdr(headers, HEADER_COALITION_NONCE),
...
test("MT verifies a Coalition signature over the exact request", () => {
  ...
  assert.ok(checkFreshness(hdr(h, HEADER_COALITION_TIMESTAMP)));
  assert.ok(
    verifyRequest(
      rebuild(h, "POST", "/api/agent/lifecycle", body),
      hdr(h, HEADER_COALITION_SIGNATURE),
      publicKeyBase64
    )
  );
});
...
test("both issued key forms load: MT's raw 32-byte seed and a PKCS#8 PEM", () => {
  for (const { publicKeyBase64, value } of [seedKeypair(), pemKeypair()]) {
    ...
    assert.ok(
      verifyRequest(rebuild(h, "GET", "/api/agent/nodes", ""), hdr(h, HEADER_COALITION_SIGNATURE), publicKeyBase64),
...
test("a tampered body fails: the signature covers sha256(body)", () => {
  ...
  assert.equal(
    verifyRequest(rebuild(h, "POST", "/api/agent/payment", tampered), hdr(h, HEADER_COALITION_SIGNATURE), publicKeyBase64),
    false
  );
});
...
test("path and method are covered too — a valid signature is not portable", () => {
  ...
  assert.equal(verifyRequest(rebuild(h, "GET", "/api/agent/jobs/claim", ""), hdr(h, HEADER_COALITION_SIGNATURE), publicKeyBase64), false);
  assert.equal(verifyRequest(rebuild(h, "POST", "/api/agent/nodes", ""), hdr(h, HEADER_COALITION_SIGNATURE), publicKeyBase64), false);
});
...
test("a forged slug fails — the slug is inside the signed envelope", () => {
  ...
  assert.equal(verifyRequest(rebuild(forged, "GET", "/api/agent/nodes", ""), hdr(h, HEADER_COALITION_SIGNATURE), publicKeyBase64), false);
});
...
test("the wrong provider's pubkey rejects a well-formed signature", () => {
  ...
  const h = signCoalitionRequest(key, "GET", "/api/agent/nodes", SLUG, "");
  assert.equal(verifyRequest(rebuild(h, "GET", "/api/agent/nodes", ""), hdr(h, HEADER_COALITION_SIGNATURE), theirs.publicKeyBase64), false);
});
...
test("with a signing key configured the bearer is GONE, not sent alongside", () => {
  ...
  assert.ok(hdr(h, HEADER_COALITION_SIGNATURE));
  assert.equal(hdr(h, HEADER_COALITION_SLUG), SLUG);
});
