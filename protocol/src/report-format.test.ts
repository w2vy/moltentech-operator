...
const ERR: Finding = {
  ...
  summary: "signed from an older config.env (coalitionUrl) — re-run `fh-toolkit sign`",
  fix: "fh-toolkit sign",
};
const WARN: Finding = {
  rule: "NOT_YET_FILLED",
...
test("⭐ a finding with a one-command fix is the LAST line of the report", () => {
  ...
  assert.match(lines.at(-1)!, /\[MANIFEST_STALE\] .*re-run `fh-toolkit sign`/);
...
test("⭐ the headline carries the FIX, not the first half of the diagnosis", () => {
  ...
  assert.match(line, /re-run `fh-toolkit sign`/);
...
test("the detail block still says everything, above the closing instruction", () => {
  ...
  assert.ok(
    text.indexOf("Pasting it at /onboard") < text.indexOf("re-run `fh-toolkit sign`"),
