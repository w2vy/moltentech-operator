...
export interface Answers {
   ...
   * hostname: `fh-toolkit` and the agent both run in containers, which resolve names
...
export function validateAnswers(
  ...
  errs.push(...validateHosts(a.hosts, prefixErr ? undefined : a.vmNamePrefix, minimums));
...
/**
 * The stock-take half of `validateAnswers`, on its own so `fh-toolkit inventory` — which
 * takes hosts without the rest of the answers — applies exactly the rules `init` does.
 * `vmNamePrefix` undefined = the prefix itself is invalid, so the namespace rule is
 * skipped rather than reported once per slot.
 */
export function validateHosts(
  hosts: HostAnswer[],
  vmNamePrefix: string | undefined,
  minimums: Record<string, number> = TIER_FLOORS_CENTS
): string[] {
  const errs: string[] = [];
  if (hosts.length === 0) errs.push("at least one Proxmox host is required.");
  ...
  for (const h of hosts) {
    ...
    for (const s of h.slots ?? []) {
      ...
      if (vmNamePrefix && !String(s.vmName ?? "").toLowerCase().startsWith(vmNamePrefix)) {
        errs.push(`slot ${s.vmName}: vmName must start with the VM name prefix "${vmNamePrefix}".`);
      }
...
export function renderConfigEnv(a: Answers): string {
  const lines = [
    "# config.env — generated by `fh-toolkit init`. NON-SECRET: no keys, no tokens.",
...
export function renderSecretsEnv(
  ...
  const lines = [
    "# secrets.env — generated by `fh-toolkit init`. CONTAINS SECRETS.",
    ...
    "",
    "# MANIFEST_KEY — base64 of manifest-key.pem, filled in by `fh-toolkit init`.",
    ...
    "",
    "# COALITION_SIGNING_KEY — issued by the /onboard web flow after you sign, and shown",
    "# ONCE. Store it now or it is gone. It signs your Coalition's reports to MT, and it is",
    "# REQUIRED: the Coalition refuses to start without it.",
...
export function renderEnvOperator(
  ...
  // `fh-toolkit proxmox` renders this BEFORE any host is declared: the storage lines are
  // then empty and `inventory` fills them in, the same way `keygen` fills MANIFEST_PUBKEY.
  const host = a.hosts[0];
  return [
    "# .env.operator — generated by `fh-toolkit init`. The AGENT's environment.",
    ...
    "",
    `PROXMOX_URL=${proxmox.url ?? (host ? `https://${host.nodeName ?? host.name}:8006` : "")}`,
    `PROXMOX_TOKEN_ID=${proxmox.tokenId ?? ""}`,
    `PROXMOX_TOKEN_SECRET=${proxmox.tokenSecret ?? ""}`,
    // ⚠️ These two, not inventory.json, are what a provision READS (env wins over
    // inventory); `inventory` rewrites them from the first host so the two cannot drift.
 ...
 * from the outside — `fh-toolkit doctor --check-hub` reports the deployed Coalition
...
export function renderReadme(a: Answers): string {
  ...
  L.push("");
  L.push("  THE AGENT runs HERE, on this machine, from this directory. It talks to");
  L.push("  your Proxmox and builds VMs. It only ever dials out.");
  L.push("  THE COALITION runs on the FLUX NETWORK, not here. It is your public face.");
  L.push("");
  L.push("");
  L.push("RUNNING THE AGENT");
  rule();
  L.push("");
  L.push("  In this directory (the fh-agent function comes from ~/.fh-toolkit.sh):");
  L.push("");
  L.push("    fh-agent doctor       preflight — run it before the first start");
  L.push("    fh-agent start        start it              (docker compose pull + up -d)");
  L.push("    fh-agent status       is it running?        (docker compose ps)");
  L.push("    fh-agent logs         watch it (Ctrl-C stops watching, not the agent)");
  L.push("    fh-agent logs --tail 50");
  L.push("    fh-agent stop         stop and remove it    (docker compose down)");
  L.push("    fh-agent update       take a newer build and restart onto it");
  L.push("");
  L.push("  AFTER CHANGING ANY SETTING:");
  L.push("");
  L.push("    fh-agent restart      (= docker compose up -d --force-recreate)");
  L.push("");
  L.push("");
  L.push("THE FOUR THINGS THAT GO WRONG");
  rule();
  L.push("");
  L.push("  1. You changed a setting and nothing changed.");
  L.push("     `docker compose restart` does not re-read your settings. Neither does");
  L.push("     `docker restart`. The agent comes back with the OLD values and fails in");
  L.push("     exactly the same way, so it looks like your fix did not work.");
  L.push("     Always: fh-agent restart   (= docker compose up -d --force-recreate)");
  L.push("");
  L.push("  2. Deletes and reprovisions just sit there forever.");
  L.push("     Look at the first line of the log. If it says courier=off, the agent");
  L.push("     cannot receive authorization requests and never will. It needs all");
  L.push("     three of MANIFEST_KEY, COALITION_URL and OWNER_ADDRESS set.");
  L.push("");
  ...
  L.push("");
  L.push("  Running `fh-toolkit init --force` again");
  ...
  rule();
  L.push("");
  L.push("  In the moltentech-operator repository, under docs/ :");
  L.push("");
  L.push("    fh-toolkit.md            every command, every option. Start here.");
  L.push("    FluxHub-overview.md      what each file in THIS directory is, and what");
  L.push("                             it costs to change one.");
  L.push("    operator-onboarding.md   the full from-zero walkthrough.");
  L.push("");
 ...
 *
 * Generated rather than copied out of the runbook because every mistake this file can
 * carry is silent:
 *
 *   - a `/data` mount pointed at the FILE instead of the directory pins the container to
 *     an inode that any atomic editor save detaches, after which inventory edits stop
 *     reaching the agent with no error at all ([[feedback_docker_single_file_bind_mount_inode]]);
 *   - a project name derived from the directory collides between two operator stacks in
 *     similarly-named directories ([[feedback_operator_compose_shared_project_name]]);
 *   - a missing `:ro` gives the agent write access to the one directory it reads;
 *   - `:latest` means nobody can say afterwards which image half-completed an onboarding.
 *
 * ⚠️ The reload command is `up -d --force-recreate`, NOT `restart`. Whether compose
 * notices a changed env_file's CONTENTS varies by version, and `docker compose restart`
 * never re-reads it — the same trap as `docker restart` with `--env-file`, which is the
 * single most common reason a corrected key keeps returning 401.
 */
export function renderAgentCompose(a: Answers): string {
  return [
    `# The Flux Hub agent for ${a.providerName}. Generated by \`fh-toolkit init\`.`,
    "#",
    "#   fh-agent start      docker compose pull + up -d           start on the newest build (or apply a change to THIS file)",
    "#   fh-agent logs       docker compose logs -f                watch it",
    "#   fh-agent stop       docker compose down                   stop and remove",
    "#   fh-agent restart    docker compose up -d --force-recreate AFTER EDITING .env.operator  ← see below",
    "#   fh-agent update     docker compose pull, then the line above",
    "#",
    "# ⚠️  `docker compose restart` does NOT re-read .env.operator. Neither does",
    "#    `docker restart`. Any change to a key, URL or token needs --force-recreate,",
    "#    which is what `fh-agent restart` runs — or the agent keeps running with the",
    "#    old value and keeps failing the same way.",
    "#",
    "# ⚠️  data/ is mounted as a DIRECTORY, read-only. Never mount data/inventory.json",
    "#    directly: a single-file bind mount pins the container to that file's inode, and",
    "#    most editors save by write-new-then-rename, which silently detaches the mount.",
    "#    Keep data/ scoped to inventory.json — .env.operator and manifest-key.pem must",
    "#    not be readable inside the container.",
    "",
    // Explicit, not derived from the directory name: two operator stacks in ~/pve30 and
    // ~/pve30-new would otherwise share a project and fight over one container.
    `name: fh-agent-${a.providerSlug}`,
    "",
    "services:",
    "  agent:",
    ...
    "    volumes:",
    "      - ./data:/data:ro",
    "    # No ports: the agent only dials OUT — Flux Hub on 443 and your Proxmox on 8006.",
    "    # Nothing reaches it from the internet, which is why it can hold Proxmox creds.",
    "",
  ].join("\n");
}
