...
export function renderReadme(a: Answers): string {
  ...
  rule();
  L.push("");
  L.push("  In this directory:");
  L.push("");
  L.push("    docker compose up -d          start it");
  L.push("    docker compose ps             is it running?");
  L.push("    docker compose logs -f        watch it (Ctrl-C stops watching, not the");
  L.push("                                  agent)");
  L.push("    docker compose down           stop and remove it");
  L.push("    docker compose pull           take a newer build, then up -d again");
  L.push("");
  ...
  L.push("     Always: docker compose up -d --force-recreate");
  L.push("");
...
export function renderAgentCompose(a: Answers): string {
  return [
    ...
    "#   docker compose up -d --force-recreate AFTER EDITING .env.operator  ← see below",
    "#",
    ...
    "#    or the agent keeps running with the old value and keeps failing the same way.",
    "#",
