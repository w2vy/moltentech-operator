...
| `AGENT_LOAN_OFFERS_PATH` | only if you LEND | your loan-offer **declarations**, re-read each heartbeat. You write only what you hold — which slot, who may borrow it, for how long, until when; the agent fills in your slug, the slot's tier (from `inventory.json`) and the nonce, and signs with your own key. It also checks a borrowed VM's stamp against these to work out whose loan it is and when it ends. A **local** file on purpose — FH is deliberately not on that path. Unset = you lend nothing and the loan scan is a no-op. |
